...
def entry_to_db(submit_entry, operator_id, reagent, time_elapsed, amount_pipetd):
    if submit_entry:
        ...
        connection.commit()
        # Printing the entry entered
        print("Inserted Record :- ", sample_entry)
        return "view-entry"
...
def entry_table(
    ...
    if tab == "view-entry":
        ...
        #Printing No of records in table
        #print("No of records in data_entries table :- ", len(df))
        if "add-button" in callback:
            ...
            print("Database cleared as per UI trigger")
